...
import 'package:clothes/users/order/order_now_screen.dart';
...
class _CartListScreenState extends State<CartListScreen> {
    ...
    try {
      ...
      if (res.statusCode == 200) {
        ...
        if (responseBodyOfGetCurrentUserCartItems['success'] == true) {
          (responseBodyOfGetCurrentUserCartItems['data'] as List)
  ...
  deleteSelectedItemFromUserCartlist(int cartID) async {
    try {
      var res = await http.post(Uri.parse(API.deleteItemFormCartList),
  ...
  //get selected items on cart list for order checkout
  List<Map<String, dynamic>> getSelectCartListItemsInformation(){
    List<Map<String, dynamic>> selectedItemsInformation = [];
    ...
    if(cartListController.selectedItemList.isNotEmpty){
      for (var selectedCartListItem in cartListController.cartList) {
        ...
        if(cartListController.selectedItemList.contains(selectedCartListItem.cart_id)){
          ...
          selectedItemsInformation.add(itemInformation);
        }
                  ...
                  return IconButton(
                    onPressed: () async {
                      ...
                      if (responseFromDialogBox == "yesDelete") {
                        ...
                        for (var eachselectedUserCartID in cartListController.selectedItemList) {
                          deleteSelectedItemFromUserCartlist(
                              eachselectedUserCartID);
                          // id.add(selectedItem);
                        }
        ...
        builder: (c) {
          return Container(
            decoration: const BoxDecoration(color: Colors.black, boxShadow: [
              BoxShadow(
                  offset: Offset(0, 1), color: Colors.white24, blurRadius: 6)
            ]),
                  ...
                  child: InkWell(
                    onTap: () {
                      ...
                      Get.to(OrderNowScreen(
                        selectedCartListItemsInfo: getSelectCartListItemsInformation(),
                        totalAmount: cartListController.total,
                        selectedCardIDs: cartListController.selectedItemList,
                      )) : null;
